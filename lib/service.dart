...
void restartService({VoidCallback? onRestarted}) {
  ...
  System.data.service.startService().then(
    (value) {
...
void onEvent(Map<String, dynamic>? event) {
  ...
  switch (event["action"]) {
    case ServiceValueAction.sendPosition:
      saveLocationUser(userId: event[Prefkey.userId]);
      break;
    case "sendToForeGround":
      debugPrint("send to foreground");
      System.data.service.setAsBackgroundService();
      break;
  }
}

void saveLocationUser({int? userId}) {
...
void postLocation({required Position value, int? userId}) {
  PositionModel.save(
